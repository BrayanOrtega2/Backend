...
const updateUserAPI = async (req, res) => {
  let { nombre, apellido, email, contrasena, documento, rol, estado, id } = req.body;
  let message;
  ...
  try {
    let response = await updateUser({ nombre, apellido, email, contrasena, documento, rol, estado, id });
    message = new ResponseBody(true, 200, response);
  } catch (error) {
    if (error.statusCode) {
      message = new ResponseBody(error.ok, error.statusCode, error.data);
